import {
	ComponentPropsWithRef,
	ReactElement,
	ReactNode,
	Ref,
	useCallback,
	useEffect,
	useId,
	useRef,
	useState,
} from 'react'

import Icon from './shared/Icon'
import IconButton from './IconButton'
import Spinner from './Spinner'
import { dropdownAnimation } from '@utils'
import { motion } from 'framer-motion'
import { useDebounce } from 'usehooks-ts'

export interface AutocompleteProps<T> extends ComponentPropsWithRef<'input'> {
	clearable?: boolean
	fullWidth?: boolean
	getOptionLabel?: (o) => ReactNode | ReactElement | string
	getOptionValue?: (o) => any
	groupBy?: (o) => string
	label?: string
	loading?: boolean
	loadingText?: string
	noOptionText?: string
	onChange?: (e?) => any
	onSelectAction: (e?) => any
	options?: T[]
	renderInput?: () => string | ReactElement | HTMLElement
}

const Autocomplete = <T,>({
	clearable = false,
	fullWidth = false,
	getOptionLabel,
	getOptionValue,
	groupBy,
	label,
	loading = false,
	loadingText,
	noOptionText,
	onChange,
	onSelectAction,
	options = [],
	placeholder,
	renderInput,
	...rest
}: AutocompleteProps<T> & { myRef?: Ref<HTMLInputElement> }) => {
	const id = useId()
	const ref = useRef<HTMLInputElement>(null)
	const [open, setOpen] = useState<boolean>(false)
	const [value, setValue] = useState<any>()
	const debouncedOpen = useDebounce(open, 100)

	const onInputChange = async (e) => {
		console.log(e.target)
		setValue(e.target.value)
		if (onChange) {
			onChange(e)
		}
	}

	const onSelect = useCallback(
		(option) => {
			if (getOptionValue) {
				setValue(getOptionValue(option))
			}
		},
		[getOptionValue],
	)

	const handleOpen = () => {
		setOpen(true)
		ref.current?.focus()
	}

	const handleClose = () => {
		setOpen(false)
		ref.current?.blur()
	}

	const handleClear = () => {
		onInputChange({ target: { value: '' } })
	}

	useEffect(() => {
		const handleKeydown = (e: KeyboardEvent) => {
			switch (e.key) {
				case 'Escape':
					setOpen(false)
					break
				default:
					return setOpen(true)
			}
		}

		document.getElementById(id)?.addEventListener('keydown', handleKeydown)
		document.getElementById(id)?.addEventListener('focusin', handleOpen)
		document.getElementById(id)?.addEventListener('focusout', handleClose)

		return (): void => {
			document.getElementById(id)?.removeEventListener('keydown', handleKeydown)
			document.getElementById(id)?.removeEventListener('focusin', handleOpen)
			document.getElementById(id)?.removeEventListener('focusout', handleClose)
		}
	}, [])

	const LoadingEl = () => (
		<div className="w-full flex justify-center p-2">
			<Spinner text={loadingText} />
		</div>
	)
	const RenderList = () => {
		if (!options.length) {
			return (
				<div className="w-full flex justify-center p-2">
					<span className="text-base text-gray-400">{noOptionText}</span>
				</div>
			)
		}
		return (
			<>
				{options?.map((option, idx) => (
					<div
						key={idx}
						className="w-full px-4 py-3 hover:bg-primary hover:text-white"
						onMouseDown={onSelectAction && onSelectAction(option)}
					>
						<span className="font-semibold py-2 px-6">{getOptionLabel && getOptionLabel(option)}</span>
					</div>
				))}
			</>
		)
	}

	return (
		<div className={`${!fullWidth ? 'min-w-[400px]' : ''} relative flex flex-col`}>
			<label htmlFor={id} className="">
				<span className="text-gray-400">{label}</span>
			</label>
			<div className="p-1 inline-flex items-center rounded-md bg-white border-gray-200 border-2 focus-within:border-[1px] focus-within:border-primary">
				<Icon className="mx-2" icon="search" />
				<div className="flex-grow-2 inline-flex px-1 py-0.5 rounded-md">
					<input
						ref={ref}
						tabIndex={1}
						className="w-full focus-visible:outline-none"
						id={id}
						autoComplete="on"
						type="text"
						value={value}
						placeholder={placeholder}
						onChange={onInputChange}
						{...rest}
					/>
					{clearable && !!value && (
						<IconButton onClick={handleClear} size="sm">
							<Icon icon="close" size="sm" />
						</IconButton>
					)}
				</div>
				{debouncedOpen ? (
					<IconButton onClick={handleClose}>
						<Icon icon="keyboard_arrow_up" />
					</IconButton>
				) : (
					<IconButton onClick={handleOpen}>
						<Icon icon="keyboard_arrow_down" />
					</IconButton>
				)}
			</div>

			{debouncedOpen && (
				<motion.div
					initial="closed"
					animate="open"
					exit="exit"
					variants={dropdownAnimation}
					className="absolute top-full rounded-b w-full origin-top-left bg-white mt-1 shadow-lg max-h-[500px] overflow-y"
				>
					{loading ? <LoadingEl /> : <RenderList />}
				</motion.div>
			)}
		</div>
	)
}

export default Autocomplete
