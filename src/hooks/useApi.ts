...
const useApi = (): UseApiOutput => {
    ...
    return {
        createActivity: (event: SportEvent) =>
        ...
        getActivities: () => async (dispatch: Dispatch): Promise<any> => {
        ...
        registerToActivity: (event: SportEvent) => async (dispatch: Dispatch): Promise<any> => {
            const updatedItem = await updateItem({
                endpoint: 'events',
                id: event.id,
                data: { userId: account?.id },
        ...
        getLocales: () => async (dispatch: Dispatch): Promise<any> => {
        ...
        getSports: () => async (dispatch: Dispatch): Promise<any> => {
            const items = await fetchItems({
                endpoint: 'sports',
                secure: false
            })(dispatch)
